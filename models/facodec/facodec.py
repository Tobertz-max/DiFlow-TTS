...
class FACodecEncoder(nn.Module):
    default_ckpt = os.path.join(os.path.dirname(__file__), 'checkpoints', 'ns3_facodec_encoder.bin')
    
    @classmethod
    def from_pretrained(cls, cfg, ckpt_path=None):
        
        # init model
        encoder = FACodecEncoder(
            ngf=cfg['ngf'],
            up_ratios=cfg['up_ratios'],
        ...
        if ckpt_path:
            ...
            ckpt = torch.load(cls.default_ckpt, map_location=cfg['device'], weights_only=True)
...
class FACodecDecoder(nn.Module):
    default_ckpt = os.path.join(os.path.dirname(__file__), 'checkpoints', 'ns3_facodec_decoder.bin')
    ...
    def from_pretrained(cls, cfg, ckpt_path=None):
        ...
        decoder = FACodecDecoder(
            ...
            ngf=cfg['ngf'],
            up_ratios=cfg['up_ratios'],
            vq_num_q_c=cfg['vq_num_q_c'],
            vq_num_q_p=cfg['vq_num_q_p'],
            vq_num_q_r=cfg['vq_num_q_r'],
            vq_dim=cfg['vq_dim'],
            codebook_dim=cfg['codebook_dim'],
            codebook_size_prosody=cfg['codebook_size_prosody'],
            codebook_size_content=cfg['codebook_size_content'],
            codebook_size_residual=cfg['codebook_size_residual'],
            use_gr_x_timbre=cfg['use_gr_x_timbre'],
            use_gr_residual_f0=cfg['use_gr_residual_f0'],
            use_gr_residual_phone=cfg['use_gr_residual_phone'],
        )
